...
<template>
  <nav
    class="no-print fixed left-0 right-0 top-0 z-50 border-b border-border/60 bg-background/75 backdrop-blur-md"
  >
    <div class="container flex h-16 items-center justify-between">
      ...
      <button
        type="button"
        class="text-foreground transition-transform duration-200 active:scale-95 lg:hidden"
        :aria-expanded="open"
    ...
    <div
      v-show="open"
      class="overflow-hidden border-b border-border bg-background lg:hidden"
    >
      <div class="flex flex-col gap-1 px-6 pb-5 pt-1">
        <component
          ...
          v-bind="
            link.to.startsWith('/#') ? { href: link.to } : { to: link.to }
          "
          class="py-2.5 font-body text-sm font-medium transition-colors duration-200"
          :class="
            isActive(link.to)
              ? 'text-foreground'
              : 'text-muted-foreground hover:text-foreground'
