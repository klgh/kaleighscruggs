<script setup lang="ts">
  ...
  const visibleFilters = computed(() =>
    filters.filter(
      (filter) => filter.value === "all" || categoriesWithPosts.value.has(filter.value)
    )
  );
...
<template>
  <section id="garden" class="no-print pb-24 md:pb-28" :class="isHomePreview ? 'pt-10 md:pt-14' : 'pt-16 md:pt-20'">
    <div class="container">
      <div class="mb-10 flex flex-col gap-6 md:mb-12 md:flex-row md:items-end md:justify-between">
        <div ref="headerEl" class="max-w-xl transition-all duration-500"
...
<template v-else>
        ...
        <div v-if="!isHomePreview && visibleFilters.length > 1" class="flex flex-wrap gap-x-5 gap-y-2" role="tablist"
          aria-label="Filter posts">
          <button v-for="f in visibleFilters" :key="f.value" type="button" role="tab"
            :aria-selected="active === f.value" class="font-body text-sm transition-colors duration-200" :class="active === f.value
        ...
        <RouterLink v-else-if="isHomePreview" to="/garden"
      ...
      <p v-if="filtered.length === 0" class="font-body text-muted-foreground">
        <template v-if="isHomePreview">
          ...
          <RouterLink to="/garden" class="underline underline-offset-4 hover:text-accent">browse the garden</RouterLink>
          .
        </template>
      ...
      <div v-else-if="isHomePreview" class="grid gap-10 border-t border-foreground/15 pt-8 sm:grid-cols-2 sm:gap-x-12">
        <RouterLink v-for="post in filtered" :key="post.slug" :to="`/garden/${post.slug}`" class="group flex flex-col">
          <div class="mb-4 flex items-center gap-3">
            <span :class="tagClass[post.meta.category]">{{ post.meta.category }}</span>
            <span v-if="post.meta.date" class="font-body text-xs text-muted-foreground">
              {{ post.meta.date }}
            </span>
          </div>
          <h3
          ...
          <p class="mb-5 max-w-[40ch] flex-1 font-body text-sm leading-relaxed text-muted-foreground">
            {{ post.meta.excerpt }}
          </p>
          <span
      ...
      <div v-else class="grid gap-10 lg:grid-cols-12 lg:items-start lg:gap-12">
        <div v-if="lead" class="lg:col-span-7">
          <RouterLink :to="`/garden/${lead.slug}`"
            class="group block border-t border-foreground/15 pt-8 transition-colors duration-200 hover:border-accent">
            <div class="mb-5 flex items-center gap-3">
              <span :class="tagClass[lead.meta.category]">{{ lead.meta.category }}</span>
              <span v-if="lead.meta.date" class="font-body text-xs text-muted-foreground">
                {{ lead.meta.date }}
              </span>
            </div>
            <h3
            ...
            <p class="mb-6 max-w-[48ch] font-body text-base leading-relaxed text-muted-foreground">
              {{ lead.meta.excerpt }}
            </p>
            <span
              class="inline-flex items-center gap-1.5 font-body text-sm font-medium text-foreground transition-transform duration-200 group-hover:translate-x-0.5">
        ...
        <div class="flex flex-col lg:col-span-5">
          <RouterLink v-for="post in rest" :key="post.slug" :to="`/garden/${post.slug}`"
            ...
            <p class="font-body text-sm leading-relaxed text-muted-foreground line-clamp-2">
              {{ post.meta.excerpt }}
            </p>
          </RouterLink>
        </div>
